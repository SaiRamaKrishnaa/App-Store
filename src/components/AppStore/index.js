...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredAppsList = appsList.filter(
      eachObj =>
        eachObj.category === tabId &&
        eachObj.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="app-store-container">
        <h1 className="app-store-heading">App Store</h1>
        <div className="app-store-search-input-container">
          <input
            type="search"
        ...
        <ul className="app-store-tabsList-container">
          {tabsList.map(eachObj => (
            <TabItem
              key={eachObj.tabId}
              tabDetails={eachObj}
              isActive={eachObj.tabId === tabId}
              onSelect={this.onSelect}
            />
          ))}
        </ul>

        <ul className="app-store-appsList-container">
          {filteredAppsList.map(eachObj => (
            <AppItem key={eachObj.appId} appDetails={eachObj} />
          ))}
        </ul>
      </div>
    )
